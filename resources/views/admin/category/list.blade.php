...
<div class="row">
    <table class="table table-hover">
        ...
        <tr class="table-danger table-warning table-success table-info">
            ...
            <td>
                <a href="{{ url('/admin/category/modify').'?id='.$item['cat_id'] }}">修改</a>
                <a href="{{ url('/admin/category/delete').'?id='.$item['cat_id'] }}">删除</a>
            </td>
        </tr>
        @endforeach
        </tbody>
    ...
    <div>
        @for ($i = 1; $i <= $pagination['total_page']; $i++)
            <a class="btn btn-general btn-white" href="{{ url('admin/category/list').'?page='.$i }}">{{ $i }}</a>
        @endfor
    </div>
</div>
@endsection('content')
